...
# Your name: Elli
# Your student id: 
# Your email: ehoke@umich.edu
# Who or what you worked with on this homework (including generative AI like ChatGPT): Claude
# If you worked with generative AI also add a statement for how you used it.
# e.g.: 
# Asked Cluade for help debugging and understanding the JSON structure
#
# Did your use of GenAI on this assignment align with your goals and guidelines in your Gen AI contract? If not, why?
# Yes I used GenAI to help me understand the JSON structure and debug my code, which is in line with my goals and guidelines in my Gen AI contract.
#
...
def get_longest_lifespan_breed(cache_file):
    ...
    cache = load_json(cache_file)
    longest_lifespan = -1
    longest_breed_name = None

    for breed in cache.values():
        try:
            breed_name = breed['data']['attributes']['name']
            max_life = breed['data']['attributes']['life']['max']
            if isinstance(max_life, int):
                if max_life > longest_lifespan:
                    longest_lifespan = max_life
                    longest_breed_name = breed_name
                elif max_life == longest_lifespan and breed_name < longest_breed_name:
                    longest_breed_name = breed_name
        except KeyError:
            continue
...
def get_groups_above_cutoff(cutoff, cache_file):
    ...
    for breed in cache.values():
        try:
            group_id = breed['data']['relationships']['group']['data']['id']
            group_count[group_id] = group_count.get(group_id, 0) + 1
        except KeyError:
            continue
    return {group_id: count for group_id, count in group_count.items() if count >= cutoff}
...
def recommend_breeds_in_same_group(breed_name, cache_file):
    ...
    for breed in cache.values():
        try:
            if breed['data']['attributes']['name'].lower() == breed_name.lower():
                target_breed = breed
                break
        except KeyError:
            continue
    ...
    try:
        target_group_id = target_breed['data']['relationships']['group']['data']['id']
    except KeyError:
    ...
    for breed in cache.values():
        try:
            if breed['data']['relationships']['group']['data']['id'] == target_group_id and breed['data']['attributes']['name'].lower() != breed_name.lower():
                recommendations.append(breed['data']['attributes']['name'])
        except KeyError:
            continue    
